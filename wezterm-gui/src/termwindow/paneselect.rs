...
use config::keyassignment::{KeyAssignment, PaneSelectArguments, PaneSelectMode, ConfirmPromptMode, ConfirmPromptArguments};
use config::Dimension;
use mux::Mux;
use mux::tab::PositionedPane;
use std::borrow::Borrow;
use std::cell::{Ref, RefCell};
use wezterm_term::{KeyCode, KeyModifiers, MouseEvent, CellAttributes};
...
impl ConfirmPrompt {
    pub fn new(term_window: &mut TermWindow, pane_id: usize ,args: &ConfirmPromptArguments) -> Self {
    ...
    fn compute(
        ...
        prompt: &str,
        _pane_id: usize,
    ) -> anyhow::Result<(Vec<ComputedElement>, String)> {
        let font = term_window
            .fonts
            .confirm_prompt_font()
            .expect("to resolve confirm prompt font");
        let metrics = RenderMetrics::with_font_metrics(&font.metrics());
        ...
        let pos = term_window.get_panes_to_render().to_owned().iter().find(|p| p.is_active).unwrap().clone();
        // let posp = term_window.get_active_pane_or_overlay().unwrap();
        // let pos = PositionedPane::from(posp);

        let palette = term_window.palette();
        let mut elements = vec![];
        let caption = crate::tabbar::parse_status_text(prompt.clone(), CellAttributes::default());
        let element = Element::new(&font, ElementContent::Text(caption.as_str()))
            .colors(ElementColors {
                border: BorderColor::new(
                    term_window.config.pane_select_bg_color.to_linear().into(),
                ),
    ...
    fn perform_selection(
        ...
        if term_window.tab_state(tab_id).overlay.is_none() {
            ...
            match self.mode {
                ConfirmPromptMode::AnyKey => {
                }
                ConfirmPromptMode::YesNo => {
                }
            }
        }
...
impl Modal for ConfirmPrompt {
    ...
    fn computed_element(
        ...
        if self.element.borrow().is_none() {
            let (element, labels) = Self::compute(term_window, &self.prompt, self.pane_index)?;
            self.element.borrow_mut().replace(element);
        }
        Ok(Ref::map(self.element.borrow(), |v| {
            v.as_ref().unwrap().as_slice()
